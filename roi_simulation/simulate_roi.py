...
print(cwd)
master_loc = '/nfs/astrop/n1/kuhlmann/NGC_1275/ts_limit/roi_simulation/roi_files/fits_01'
with open(cwd+'/config_local.yaml', 'r') as i:
    config = yaml.safe_load(i)

if 'n1/kuhlmann' in cwd:
    workdir = f'/nfs/astrop/n1/kuhlmann/NGC_1275/ts_limit/roi_simulation/roi_files/roi_{input_num}'
    try:
        os.mkdir(workdir)
...
gta = GTAnalysis.create(f'{workdir}/fit_newminuit.npy', cwd+'/config_modified.yaml')
print('loaded loglike:', -gta.like())
roi_npy = np.load(f'{workdir}/fit_newminuit.npy', allow_pickle=True).flat[0]
